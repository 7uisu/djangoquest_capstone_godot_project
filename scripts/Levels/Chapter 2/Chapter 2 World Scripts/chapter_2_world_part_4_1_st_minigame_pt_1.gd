#chapter_2_world_part_4_1st_minigame_pt1.gd
extends Node2D

@onready var delivery_bot = $YSortLayer/DeliveryBot2
@onready var player = $Player # Get reference to Player node
@onready var instructions_ui = $UILayer/Chapter2Minigame1Instructions

@onready var player_camera = $Player/Camera2D 
@onready var delivery_bot_camera = $YSortLayer/DeliveryBot2/Camera2D 
...
func setup_minigame():
	...
	# Disable Player's input processing so Player won't move
	if player:
	...
	if delivery_bot_camera and is_instance_valid(delivery_bot_camera):
		...
		print("Chapter2Minigame1: DeliveryBot2 camera made current.")
	else:
		printerr("Chapter2Minigame1: DeliveryBot2's Camera2D not found or invalid!")

	# Enable DeliveryBot2 movement/input
	if delivery_bot and is_instance_valid(delivery_bot):
		delivery_bot.enable_movement()
		print("Chapter2Minigame1: DeliveryBot2 movement enabled.")
	else:
		printerr("Chapter2Minigame1: DeliveryBot2 node not found or invalid!")
...
func minigame_completed():
	...
	# Disable delivery bot movement after completion
	if delivery_bot:
		delivery_bot.disable_movement()
		print("Chapter2Minigame1: DeliveryBot2 movement disabled.")
...
func reset_minigame():
	...
	if delivery_bot:
		...
		delivery_bot.disable_movement()
		print("Chapter2Minigame1: DeliveryBot2 reset and disabled.")
