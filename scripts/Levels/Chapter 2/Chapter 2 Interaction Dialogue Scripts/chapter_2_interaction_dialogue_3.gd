# chapter_2_interaction_dialogue_3.gd
extends Control
...
@onready var player = get_node_or_null("/root/Playground/Player") # Using get_node_or_null for safety

# --- DIALOGUE CONTENT FOR DIALOGUE 3 ---
# !!! REPLACE THIS WITH YOUR ACTUAL DIALOGUE !!!
var dialogue_data = {
	"initial_greeting_part_3": {
		"dialogue": [
			{"speaker": "You", "text": "...Ugh. I’m... I'm okay, I think."},
			{"speaker": "Pip", "text": "Are you hurt?! Say something!"},
			{"speaker": "You", "text": "Just... a little sore. That was close."},
			{"speaker": "Narrator", "text": "You slowly get to your feet. The room is dimly lit, ancient mechanisms ticking softly around you."},
			{"speaker": "Pip", "text": "Where are we now...? This doesn’t look like the upper floors."},
			{"speaker": "You", "text": "No... but maybe there's a way forward. Or... a way back."},
			{"speaker": "Pip", "text": "Let’s be careful. Something tells me this place has more surprises waiting."}
		]
	}
}
...
var dialogue_sequence_keys: Array = ["initial_greeting_part_3"]
var current_sequence_index: int = 0

func _ready() -> void:
	# Ensure player reference is valid
	if not player:
		printerr("Chapter2InteractionDialogue3: Player node not found at /root/Playground/Player. Movement will not be disabled/enabled.")
...
func start_dialogue() -> void:
	...
	if dialogue_sequence_keys.size() > 0:
		...
		printerr("Chapter2InteractionDialogue3: No dialogue sequences defined!")
...
func start_dialogue_sequence(sequence_key: String) -> void:
	if not dialogue_data.has(sequence_key):
		printerr("Chapter2InteractionDialogue3: Dialogue sequence key not found: '", sequence_key, "'")
		finish_dialogue()
		return
	...
	if current_dialogue_lines.size() > 0:
		...
		printerr("Chapter2InteractionDialogue3: Dialogue sequence '", sequence_key, "' has no lines.")
		finish_dialogue()
...
func display_current_line() -> void:
	...
	if current_image_paths.size() > current_line_index and current_image_paths[current_line_index] != "":
		...
		if loaded_image:
			...
			printerr("Chapter2InteractionDialogue3: Failed to load image: ", image_path)
			texture_rect.visible = false
	else:
		texture_rect.visible = false
...
func finish_dialogue() -> void:
	print("[Chapter2InteractionDialogue3] Dialogue finished.")
	if player:
