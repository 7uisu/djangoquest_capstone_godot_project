# chapter_2_interaction_dialogue_8.gd
extends Control
...
# --- DIALOGUE CONTENT FOR DIALOGUE 8 (FLYING ESCAPE) ---
var dialogue_data = {
	"escape": {
		"dialogue": [
			{"speaker": "GRIT", "text": "Away we go!"},
			{"speaker": "Pip", "text": "Whoa! This is amazing!"},
			{"speaker": "You", "text": "I can't believe we're actually flying!"},
			{"speaker": "URL Bot Red", "text": "The view from up here is incredible!"},
		...
		# Set the escape flight image for ALL dialogue lines
		"background_image": "res://textures/Plain Color BG/Sky-Blue.png"
	}
}
...
var dialogue_sequence_keys: Array = ["escape"]
var current_sequence_index: int = 0

func _ready() -> void:
	self.visible = false # Start hidden
	if not player:
		printerr("Chapter2InteractionDialogue8: Player node not found. Movement control might fail.")
...
func start_this_dialogue(sequence_to_play: String = "") -> void:
	if not player:
		printerr("Chapter2InteractionDialogue8: Player node not found when trying to start dialogue.")
	...
	if key_to_start == "" and dialogue_sequence_keys.size() > 0:
		...
		printerr("Chapter2InteractionDialogue8: No sequence specified and no default sequences available.")
		finish_dialogue()
		return
...
func start_dialogue_sequence(sequence_key: String) -> void:
	if not dialogue_data.has(sequence_key):
		printerr("Chapter2InteractionDialogue8: Dialogue sequence key not found: '", sequence_key, "'")
		finish_dialogue()
		return
	...
	if current_dialogue_lines.size() > 0:
		...
		printerr("Chapter2InteractionDialogue8: Dialogue sequence '", sequence_key, "' has no lines.")
		finish_dialogue()
	...
	if current_background_image != "":
		if current_background_image.begins_with("res://"):
			...
			if loaded_image:
				...
				print("[Chapter2InteractionDialogue8] Background image loaded: ", current_background_image)
			else:
				printerr("Chapter2InteractionDialogue8: Failed to load background image: ", current_background_image)
				texture_rect.visible = false
		else:
			texture_rect.visible = false
	else:
...
func finish_dialogue() -> void:
	print("[Chapter2InteractionDialogue8] Escape dialogue finished.")
	if player:
	...
	var next_scene_path = "res://scenes/Levels/Chapter 2/chapter_2_world_part_7.tscn"
