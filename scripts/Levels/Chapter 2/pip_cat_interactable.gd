...
func _ready():
	...
	if anim_sprite:
		if anim_sprite.sprite_frames != null:
			...
			if "interact" in animation_names:
				anim_sprite.play("interact")
			elif "pip_idle_right_2" in animation_names: # added this
				anim_sprite.play("pip_idle_right_2")
			else:
				print("Error:  Neither 'interact' nor 'pip_idle_right_2' animation found.")
...
func interact():
	...
	print("You have interacted with the Pip")
	has_interacted = true
